...
folder = os.path.join(os.getcwd(), "Neighbourhood", f"{args.dataset}") 
...
medoid_indices = cond.condensation(knn, k_nn, y_train_or, gower_train, max_size = args.max_size)
print("Medoids", len(medoid_indices))
...
y_cl_0 = np.where(y_train_or[medoid_indices] == 0)[0]
y_cl_1 = np.where(y_train_or[medoid_indices] == 1)[0]

y_cl_0 = medoid_indices[y_cl_0]
y_cl_1 = medoid_indices[y_cl_1]

k=args.k

knn_0 = NearestNeighbors(metric='precomputed', n_neighbors=k).fit(gower_train[y_cl_0][:, y_cl_0])
knn_1 = NearestNeighbors(metric='precomputed', n_neighbors=k).fit(gower_train[y_cl_1][:, y_cl_1])
...
for idx in range(neigh.shape[0]):
    ...
    X_knn_same = np.zeros(shape = (x.shape[0], k, X_test_or.shape[1]))
    X_knn_other = np.zeros(shape = (x.shape[0], k, X_test_or.shape[1]))
    ...
    gower_same_mean = np.nanmean(gower_same, axis=1)
    gower_other_mean = np.nanmean(gower_other, axis=1)

    results[idx, : , :] = gower_other_mean - gower_same_mean
...
np.save(os.path.join(folder, f"knn_attributions_condensation_{k}"), results) #salvati a norma 1
np.save(os.path.join(folder, f"knn_neigh_size_condensation_{k}"), neigh_size)
...
np.save(os.path.join(folder, f"knn_robustness_condensation_{k}"), robustness)
